...
function normalizeSettings(settings) {
  ...
  const allowedSpeedTiers = new Set(["standard", "fast"]);
  const speedTier = allowedSpeedTiers.has(next.speedTier)
    ? next.speedTier
    : (next.fastMode ? "fast" : DEFAULT_CHAT_SETTINGS.speedTier);
...
function normalizeBoardSettings(settings) {
  ...
  const allowedAgentRunners = new Set(["codex", "grok", "kilo"]);
  const allowedModelProviders = new Set(["codex", "xai", "openrouter"]);
  ...
  return {
    ...
    agentRunner: allowedAgentRunners.has(next.agentRunner) ? next.agentRunner : DEFAULT_BOARD_SETTINGS.agentRunner,
    modelProvider: allowedModelProviders.has(next.modelProvider) ? next.modelProvider : DEFAULT_BOARD_SETTINGS.modelProvider,
...
function trimChatForStorage(chat) {
  ...
  return {
    ...
    editingOriginalMessages: Array.isArray(source.editingOriginalMessages)
      ? source.editingOriginalMessages.slice(-80).map(trimMessageForStorage)
      : [],
    ...
    messages: Array.isArray(source.messages)
      ? source.messages.slice(-80).map(trimMessageForStorage)
      : []
  };
}

function trimMessageForStorage(item) {
  const source = item || {};
  return {
    ...
    runStartedAt: Number(source.runStartedAt || 0),
    runFinishedAt: Number(source.runFinishedAt || 0),
    raw: source.raw ? String(source.raw).slice(0, MAX_ATTACHMENT_BYTES) : "",
    attachments: Array.isArray(source.attachments)
      ? source.attachments.slice(-20).map((item) => ({
          id: String(item.id || ""),
          name: String(item.name || "file"),
          path: String(item.path || ""),
          relativePath: String(item.relativePath || ""),
      ...
      : [],
    changes: Array.isArray(source.changes) ? source.changes.slice(0, 20).map((change) => ({
