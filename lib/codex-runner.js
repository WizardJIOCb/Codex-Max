...
class CodexRunner {
  ...
  run(chatId, prompt, sessionId, settings, board, projectPath, boardSettings) {
    if (!chatId || !prompt) {
      return;
    }
    ...
    const args = buildCodexArgs({
      ...
      modelProvider: normalizeModelProviderSettings(boardSettings),
      cwd
    });
...
function buildCodexArgs({ sessionId, settings, modelProvider, cwd }) {
  const args = ["exec"];
  ...
  if (sessionId) {
    ...
    pushConfigOverrides(args, settings, modelProvider, true);
    args.push(sessionId, "-");
    return args;
  }
  ...
  pushConfigOverrides(args, settings, modelProvider, false);
  args.push("-");
  return args;
}

function pushConfigOverrides(args, settings, modelProvider, isResume) {
  pushModelProviderOverrides(args, modelProvider);
  ...
  if (settings.speedTier === "fast" && (!modelProvider || modelProvider.id === "codex")) {
...
function pushModelProviderOverrides(args, modelProvider) {
  if (!modelProvider || modelProvider.id === "codex") {
    return;
  }

  args.push("-c", `model_provider=${tomlString(modelProvider.id)}`);
  args.push("-c", `model_providers.${modelProvider.id}.name=${tomlString(modelProvider.name)}`);
  args.push("-c", `model_providers.${modelProvider.id}.base_url=${tomlString(modelProvider.baseUrl)}`);
  args.push("-c", `model_providers.${modelProvider.id}.env_key=${tomlString(modelProvider.envKey)}`);
}

function normalizeModelProviderSettings(settings) {
  const provider = String(settings && settings.modelProvider || "codex").toLowerCase();
  if (provider === "xai") {
    return {
      id: "xai",
      name: "xAI",
