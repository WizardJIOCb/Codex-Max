...
function resolveKiloExecutable(configured) {
  const wantsAuto = !configured || configured === "kilo";
  const configuredPath = wantsAuto ? "" : stripQuotes(configured);
  ...
  for (const candidate of getKiloCandidates()) {
    if (candidate && fs.existsSync(candidate)) {
      return {
        command: candidate,
        shell: shouldUseShell(candidate)
...
function getKiloCandidates() {
  ...
  for (const root of getExtensionRoots()) {
    ...
    for (const entry of entries) {
      if (!entry.isDirectory() || !/^kilocode\.kilo-code-/i.test(entry.name)) {
        continue;
      }

      candidates.push(path.join(root, entry.name, "bin", process.platform === "win32" ? "kilo.exe" : "kilo"));
    }
  }
  ...
  if (process.platform === "win32") {
    ...
    if (appData) {
      ...
      candidates.push(path.join(appData, "npm", "kilo.exe"));
      candidates.push(path.join(appData, "npm", "kilocode.cmd"));
      candidates.push(path.join(appData, "npm", "kilocode"));
      candidates.push(path.join(appData, "npm", "kilocode.exe"));
    }
  } else {
    const home = os.homedir();
    candidates.push(path.join(home, ".npm-global", "bin", "kilo"));
    candidates.push(path.join(home, ".npm-global", "bin", "kilocode"));
    candidates.push("/usr/local/bin/kilo");
    candidates.push("/usr/local/bin/kilocode");
    candidates.push("/opt/homebrew/bin/kilo");
    candidates.push("/opt/homebrew/bin/kilocode");
  }
